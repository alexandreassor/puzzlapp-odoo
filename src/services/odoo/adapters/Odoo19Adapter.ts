/**
 * Adaptateur specifique pour Odoo 19
 *
 * Particularites Odoo 19:
 * - Champ 'origin' sur stock.move (pas 'name')
 * - Champ 'picked' disponible
...
export class Odoo19Adapter extends BaseOdooAdapter {
  ...
  readonly versionConfig: VersionConfig = {
    locations: {
      suppliers: 1,   // Vendors/Fournisseurs
      stock: 5,       // WH/Stock
      customers: 2    // Customers
    },
    fields: {
      stockMoveReference: 'origin',  // En v19, on utilise 'origin' pas 'name'
      stockMoveDescription: 'origin'
    },
    features: {
      hasPickedField: true,
  ...
  async getStockMoves(filters: StockMoveFilters = {}): Promise<OdooStockMove[]> {
    ...
    const moves = await this.callOdoo<Array<{
      ...
      fields: ['id', 'date', 'reference', 'product_id', 'product_qty', 'price_unit',
    ...
    return moves.map(m => ({
      ...
      origin: m.origin !== false ? m.origin : null,
      state: m.state
    }))
  }
  ...
  async createStockEntry(data: StockEntryData): Promise<ApiResult<{ moveId: number }>> {
    try {
      ...
      const moveData = {
        origin: data.reference || `Entree stock - ${new Date().toISOString()}`,
        product_id: data.productId,
        product_uom_qty: data.quantity,
        quantity: data.quantity,  // Specifique v19
        product_uom: 1,
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Erreur creation entree'
      }
    }
  }
  ...
  async createStockExit(data: StockEntryData): Promise<ApiResult<{ moveId: number }>> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Erreur creation sortie'
      }
    }
  }
  ...
  async updateStockMovePrice(moveId: number, newPrice: number): Promise<ApiResult<void>> {
    try {
      ...
      console.log(`[Odoo19Adapter] Produit ${productId} - Methode: ${costMethod}, Valorisation: ${valuation}`)
      ...
      if (!shouldSkipCumpUpdate) {
        ...
        if (totalQty > 0) {
          ...
          console.log(`[Odoo19Adapter] CUMP mis a jour: ${newCUMP.toFixed(2)}`)
        }
      } else {
        console.log(`[Odoo19Adapter] Mode AVCO + temps reel - standard_price NON modifie`)
      }
